"""
Career Co-Pilot Pro — Operator App
Full operator cockpit: batch runner, API console, admin tooling, debug surfaces.
Launched when APP_MODE=operator (or directly via: streamlit run ui/operator_app.py)
For the guided candidate UX, see ui/candidate_app.py (APP_MODE=candidate).
"""
...
def run():
    ...
    st.title("🚀 Career Co-Pilot Pro — Operator")
    st.caption("Operator mode — full cockpit. For candidate-facing guided UX run with APP_MODE=candidate or open ui/candidate_app.py")
...
- **CLI apply:** `scripts/apply_linkedin_jobs.py` — Playwright traces/screenshots depend on your runner flags and cwd (see `scripts/` and `docs/DEPLOY.md`).
    ...
    with tab6:
        _operator_mode = os.getenv("APP_MODE", "operator").lower() == "operator"
        if not _operator_mode:
            st.info("ATS / API console is only available in operator mode. Set **APP_MODE=operator** to enable it.")
            st.stop()
