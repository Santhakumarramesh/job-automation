...
10. **Follow-up digest** — `GET /api/follow-ups/digest` returns `{ text, items, count }` for due reminders (paste into email/Slack). Admin: `GET /api/admin/follow-ups/digest`. CLI: `PYTHONPATH=. python scripts/follow_up_digest.py`. Optional SMTP: `scripts/email_follow_up_digest.py` + `FOLLOW_UP_*` in `.env`. See [docs/FOLLOW_UPS.md](docs/FOLLOW_UPS.md).
